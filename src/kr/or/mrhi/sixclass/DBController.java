...
//데이터베이스에 관련된 내용을 컨트롤러에서 주관.
//(입력)->(실행결과)->(실행결과 리턴받아서 Console(UI분야)에서 출력한다.)
public class DBController {
	...
	public static List<PhoneBook> phoneBooksearchTBL(String searchData, int searchNumber) {
		final int PHONE = 1, NAME = 2, GENDER = 3;
		...
		try {
			switch(searchNumber) {
				case PHONE : searchQuery = "select * from phonebooktbl where phoneNumber like ?"; break;
				case NAME : searchQuery = "select * from phonebooktbl where name like ?";		 break;
				case GENDER : searchQuery = "select * from phonebooktbl where gender like ?";	 break;
				default : 
					System.out.println("검색번호 범위에 벗어났습니다. (다시 입력 요망)");
					return list;
			}
	...
	//4. 전화번호부 수정하기
	public static int phoneBookUpdate(String phoneNumber, String name) {
		//데이타베이스에 연결
		Connection con = DBUtility.getConnection();
